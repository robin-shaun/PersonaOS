...
- 状态：已评审的实施基线；M1、M2、M3、M4、M5、M6 已实现
- 日期：2026-07-25
- 仓库基线：`37daaa3` (`main`)
- 适用范围：第一阶段“数字员工”向第二阶段“证据驱动的数字分身”演进

> 实施进展（2026-07-25）：仓库版本已进入 `0.12.0`。M1 的审核优先资料入口与
...
> 快照及可执行 release gate。M6 加入可信本地账户、可撤销会话、服务端 actor、
> CSRF/Origin、近期再认证、应用层全领域 owner 过滤、PostgreSQL RLS 与显式
> 0.11 单 owner 迁移。第 2–5 节仍保留审计当时的 `0.6.0` 事实快照，避免用后来
> 实现篡改基线。
...
交付受信主机 CLI 账户初始化、Argon2id 密码摘要、可撤销 opaque Cookie、
CSRF/Origin 防护、近期再认证、服务端派生 actor、全领域 owner 查询约束和
PostgreSQL RLS。两个账户必须分别完成完整证据闭环，并双向证明人物、资料、记忆、
任务、会话、引用、导出和审计均不能越权；0.11 的 `local-user` 数据只通过显式
预览、冲突检查、回执和原子回滚迁移。

每个里程碑独立可运行、可测试、可演示。不得为了后续里程碑提前创建没有行为的
空目录或大量 TODO。

## 15. M1、M2、M3、M4、M5、M6 实施结果与唯一下一里程碑
...
M6 把固定演示 principal 升级为可信本地账户边界：

1. 首个管理员和后续账户只由主机受信 CLI 或已登录管理员创建，密码不接受
   argv/env，使用 Argon2id 保存摘要；
2. 登录签发 256-bit opaque Cookie，只持久化 HMAC 摘要；会话具备空闲/绝对
   过期、即时注销、登录替换和再认证轮换；
3. 浏览器写请求同时校验 SameSite=Strict Cookie、会话绑定 CSRF 和精确 Origin，
   高风险删除、原文导出、外部模型和账户创建要求近期再认证；
4. request schema 拒绝旧 `user_id` / `requested_by` 输入，所有 actor 与 owner
   均从服务端认证上下文派生；
...
8. 0.11 `local-user` 数据迁移先预览计数、唯一键冲突和非终态任务，再按精确主键
   清单原子 apply；回滚要求所有行仍与回执一致，拒绝部分恢复。
...
区分用户明示、原始资料可验证、系统总结、模型推断、用户设定和临时生成内容，
并用可追溯事件时间线展示证据、冲突、不确定性和人工修正。
